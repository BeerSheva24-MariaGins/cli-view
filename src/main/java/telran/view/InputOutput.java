	...
	default String readStringPredicate(String prompt, String errorPrompt, Predicate<String> predicate) {
		return readObject(prompt, errorPrompt, str -> {
			if (!predicate.test(str)) {
				throw new IllegalArgumentException("String does not match predicate");
			}
			return str;
		});
	}
	...
	default String readStringOptions(String prompt, String errorPrompt, HashSet<String> options) {
		return readObject(prompt, errorPrompt, str -> {
			if (!options.contains(str)) {
				throw new IllegalArgumentException("String not in options: " + options);
			}
			return str;
		});
			...
			LocalDate to) {
				return readObject(prompt, errorPrompt, str -> {
					...
					if (date.isBefore(from) || date.isAfter(to)) {
						throw new IllegalArgumentException("Date out of range: " + from + " - " + to);
					}
					return date;
				});
			}
